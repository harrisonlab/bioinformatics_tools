...
for i in $(seq 1 $Iterations); do
  ...
  if [ $Ploidy == "haploid" ]; then
    ...
    java -Xmx$Mem -jar $JavaDir/pilon-1.17.jar --threads 16 --genome assembly.fa --changes --frags assembly.fa_aligned_sorted.bam --outdir .
  elif [ $Ploidy == "diploid" ]; then
    JavaDir=/projects/oldhome/armita/prog/pilon
    java -Xmx$Mem -jar $JavaDir/pilon-1.17.jar --threads 16 --genome assembly.fa --changes --diploid --frags assembly.fa_aligned_sorted.bam --outdir .
  else
