...
for i in $(seq 1 $Iterations); do
  ...
  if [ $Ploidy == "haploid" ]; then
    ...
    pilon --threads 16 --genome assembly.fa --changes --frags assembly.fa_aligned_sorted.bam --outdir .
  elif [ $Ploidy == "diploid" ]; then
    pilon --threads 16 --genome assembly.fa --changes --diploid --frags assembly.fa_aligned_sorted.bam --outdir .
  else
